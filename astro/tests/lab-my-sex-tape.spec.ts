...
const EXPERIMENT_URL = "/lab/my-sex-tape/";
const PROCESS_URL = "/lab/my-sex-tape/process/";
...
test.describe("/lab/my-sex-tape/ — эксперимент", () => {
  test("returns 200, has my sex tape title, h1, content, no 404s", async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/my\s*sex\s*tape/i);
...
test.describe("/lab/my-sex-tape/process/ — процесс", () => {
  test("returns 200, has my sex tape title, content, no 404s", async ({ page }) => {
    const errors = await trackErrors(page);
