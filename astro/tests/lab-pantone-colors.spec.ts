...
const EXPERIMENT_URL = "/lab/pantone-colors/";
const PROCESS_URL = "/lab/pantone-colors/process/";
...
test.describe("/lab/pantone-colors/ — эксперимент", () => {
  test("returns 200, has Pantone title, h1, content, no 404s", async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/pantone/i);
...
test.describe("/lab/pantone-colors/process/ — процесс", () => {
  test("returns 200, has Pantone title, content, no 404s", async ({ page }) => {
    const errors = await trackErrors(page);
